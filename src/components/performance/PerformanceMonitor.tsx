...
export function PerformanceMonitor() {
  useEffect(() => {
    ...
    const logBundleInfo = () => {
      ...
      if (largestJS) {
        console.log(
          `📦 Largest JS: ${(largestJS.transferSize / 1024).toFixed(2)}KB - ${largestJS.name.split('/').pop()}`
        );
      }
      if (largestCSS) {
        console.log(
          `🎨 Largest CSS: ${(largestCSS.transferSize / 1024).toFixed(2)}KB - ${largestCSS.name.split('/').pop()}`
        );
      }
    };
    ...
    const logRecommendations = () => {
      ...
      if (metrics.fcp && metrics.fcp > 2000) {
        console.warn(
          '⚠️ First Contentful Paint is slow (>2s). Consider optimizing critical resources.'
        );
      }

      if (metrics.lcp && metrics.lcp > 2500) {
        console.warn(
          '⚠️ Largest Contentful Paint is slow (>2.5s). Optimize images and critical resources.'
        );
      }

      if (metrics.fid && metrics.fid > 100) {
        console.warn(
          '⚠️ First Input Delay is high (>100ms). Consider code splitting and reducing main thread work.'
        );
      }

      if (metrics.cls && metrics.cls > 0.1) {
        console.warn(
          '⚠️ Cumulative Layout Shift is high (>0.1). Add size attributes to images and reserve space for dynamic content.'
        );
      }
